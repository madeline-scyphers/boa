...
def run(config_path, client_path, num_trials, experiment_dir=None):
    ...
    if client_path:
        ...
        if not config:
            sch_jsn = load_jsonlike(client_path)
    ...
    if client_path:
        scheduler = client_from_json_file(filepath=client_path)
        if num_trials:
            scheduler.wrapper.config.orchestrator = dataclasses.replace(
                scheduler.wrapper.config.orchestrator, total_trials=num_trials
            )
        ...
        scheduler = controller.client

    if not scheduler.optimization_csv.exists() and scheduler.experiment.trials:
    ...
    if scheduler.optimization_csv.exists():
...
def exp_attach_data_from_opt_csv(metric_names, scheduler):
    df = pd.read_csv(scheduler.optimization_csv)
