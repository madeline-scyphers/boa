...
class WrappedJobRunner(Runner, metaclass=RunnerRegister):
    ...
    def run(self, trial: Trial) -> Dict[str, Any]:
        ...
        return {"job_id": trial.index, "parameterization": trial.arm.parameters}
    ...
    def poll_trial_status(self, trials: Iterable[Trial]) -> Dict[TrialStatus, Set[int]]:
        ...
        for trial in trials:
            trial_status = self.wrapper.get_trial_status(trial)
            if trial_status is not None:
                status_dict[trial_status].add(trial.index)
        return status_dict
