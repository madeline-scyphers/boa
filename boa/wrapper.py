...
class BaseWrapper(metaclass=WrapperRegister):
    ...
    def write_configs(self, trial: BaseTrial) -> None:
        """
        Writes configurations needed for launching a model run. This function defines how the parameters
        generated by boa for each trial are passed to the model.

        The behavior and output formats of this function depend on the model being used. This function should take the
        parameters generated by boa and write these to the input files needed by the model.

        Depending on the requirements of the model, this function could simply write a new configuration file for each
        file. In other cases, the model may require a new run script to be written for each trial.

        See the :class:`FETCH3 Wrapper <fetch3:fetch3.optimize.fetch_wrapper.Fetch3Wrapper>` for a
        relatively simple example where the model simply requires a new configuration file for each run.

        See the PALM wrapper for an example where a new run script needs to be generated for each run.
        #TODO add reference to palm wrapper
