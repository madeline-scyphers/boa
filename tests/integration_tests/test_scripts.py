...
def test_trial_dir_exists(branin_main_run):
    ...
    for trial_index in range(config.orchestrator.total_trials - 5):
...
def test_df(branin_main_run):
    ...
    config_metric = config.optimization.metrics["rmse"]
    ...
    assert len(df) == config.orchestrator.total_trials - 5
